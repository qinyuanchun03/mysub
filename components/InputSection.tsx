...
import { Link2, Server, Settings2, Cpu, ShieldCheck, Zap } from 'lucide-react';
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 space-y-4">
        <div className="space-y-2">
          <label className="flex items-center gap-2 text-sm font-bold text-slate-700">
            <Link2 size={16} className="text-blue-500" /> 节点链接 (VLESS/VMess/Trojan)
          </label>
          <textarea
            value={nodeLink}
            onChange={(e) => onNodeLinkChange(e.target.value)}
            placeholder="粘贴您的节点链接..."
            className="w-full h-24 p-3 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all mono"
          />
        </div>
        ...
        <div className="flex flex-wrap gap-6 pt-2">
          <label className="flex items-center gap-3 cursor-pointer group">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[10px] text-slate-400">降低首包延迟 (ed=2560)</span>
            </div>
          </label>
          ...
          <label className="flex items-center gap-3 cursor-pointer group">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[10px] text-slate-400">Encrypted Client Hello</span>
            </div>
          </label>
        </div>
