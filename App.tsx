...
import { Github, Layers, Zap, EyeOff, FlaskConical, Sparkles, ShieldCheck, AlertCircle, HelpCircle } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (uuidRegex.test(path) || fullUuidRegex.test(path)) {
      ...
      const defaultNode = `vless://${path}@${window.location.hostname}:443?encryption=none&security=tls&type=ws&host=${window.location.hostname}&path=%2F#Worker-Sub`;
      setNodeLink(defaultNode);
    } 
  }, []);

  const handleGenerate = () => {
    if (!options.quickKey && !nodeLink.trim()) {
  ...
  return (
    <div className="min-h-screen pb-20 px-4 bg-slate-50/50">
      <div className="max-w-5xl mx-auto pt-10 pb-8 flex items-center justify-between border-b border-slate-200 mb-10">
        ...
        <div className="flex items-center gap-2">
           ...
           >
             <FlaskConical size={14} className="text-amber-500" /> 加载实例节点
           </button>
           <button 
      ...
      <main className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-10">
        <div className="lg:col-span-4 space-y-6">
          <div className="p-8 bg-gradient-to-br from-slate-900 to-slate-800 rounded-[2rem] text-white shadow-2xl relative overflow-hidden">
            <div className="relative z-10 space-y-4">
               <div className="inline-flex items-center gap-2 px-3 py-1 bg-blue-500/20 rounded-full text-blue-300 text-[10px] font-bold uppercase tracking-wider">
                <Sparkles size={10} /> 修复 Nginx 伪装问题
               </div>
               <h2 className="text-3xl font-black leading-tight">绕过伪装页<br/>获取订阅信息</h2>
               <p className="text-slate-400 text-sm leading-relaxed font-medium">
                 如果访问链接看到 Nginx 页面，说明 Worker 开启了 <span className="text-white">KEY</span> 校验。请在“快速密钥”处填写您在 Worker 设置的 <span className="text-white">KEY</span>。
               </p>
            </div>
          ...
          <div className="p-6 bg-white rounded-3xl border border-slate-100 shadow-sm space-y-4">
            <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center justify-between">
              校验状态
              <HelpCircle size={14} className="cursor-help" title="确保后端 Worker 正常运行" />
            </h4>
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-medium text-slate-600">伪装绕过测试</span>
                <span className={`flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full ${options.quickKey ? 'text-emerald-500 bg-emerald-50' : 'text-amber-500 bg-amber-50'}`}>
                  {options.quickKey ? <ShieldCheck size={10} /> : <AlertCircle size={10} />}
                  {options.quickKey ? '已启用密钥' : '使用公共路径'}
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="lg:col-span-8 space-y-8">
          ...
          <button
            ...
            disabled={!isReady}
            className={`group relative w-full py-5 rounded-2xl font-black text-lg transition-all overflow-hidden ${
              isReady 
              ? 'bg-slate-900 text-white hover:bg-blue-600 shadow-2xl shadow-slate-400/20 active:scale-[0.98]' 
              : 'bg-slate-100 text-slate-400 cursor-not-allowed'
            }`}
          >
            <span className="relative z-10 flex items-center justify-center gap-3">
              {options.quickKey ? '生成密钥直连链接 (推荐)' : '生成 /sub 订阅链接'}
